...
Average). Features:
  ...
  - Each slot has four widgets: start, end, weight, clip.
      ...
      - clip (Pass L+G / Pass L / Pass G): for SDXL conditioning
...
CONDITIONING_SLOT_KEY_PATTERN = re.compile(r"^conditioning_(\d+)(_start|_end|_weight|_clip)?$")
...
def _parse_kwargs_into_per_slot_dict(kwargs):
    """Returns {slot_index: {'conditioning': list_or_None, 'start': float, 'end': float, 'weight': float, 'clip_pass_choice': str}}."""
    per_slot_dict = {}
    for key, value in kwargs.items():
        ...
        if slot_index not in per_slot_dict:
            per_slot_dict[slot_index] = {
                ...
                "end": 1.0,
                "weight": 1.0,
                "clip_pass_choice": CLIP_STREAM_PASS_BOTH_L_AND_G,
            }
        if suffix is None:
            ...
            per_slot_dict[slot_index]["clip_pass_choice"] = str(value)
    return per_slot_dict


def _apply_clip_pass_choice_mask_to_tokens_and_pooled(
    tokens_tensor, metadata_dict, clip_pass_choice
):
    """
    Returns (masked_tokens_tensor, masked_metadata_dict) per the chosen
    clip pass setting. Only acts on SDXL-shaped tensors (last dim ==
...
def _flatten_connected_slots_into_per_entry_input_records(per_slot_dict):
    ...
    for slot_index in sorted(per_slot_dict.keys()):
        ...
        clip_pass_choice = slot_data["clip_pass_choice"]
        for entry_index_within_slot, conditioning_entry in enumerate(conditioning_list):
            ...
            # input.
            entry_tokens_tensor, entry_metadata_dict = _apply_clip_pass_choice_mask_to_tokens_and_pooled(
                raw_entry_tokens_tensor, raw_entry_metadata_dict, clip_pass_choice
            )
